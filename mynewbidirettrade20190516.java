...
/*定义多货币对数量*/
#define HFOREXNUMBER 70


//定义一个买卖点最多买卖的数量
#define HSUBBUYSELLPOINTNUM 30
...
stForexIndex ForexIndex[HFOREXNUMBER];
...
stBuySellPosRecord BuySellPosRecord[HFOREXNUMBER][6*HBUYSELLALGNUM+1][HSUBBUYSELLPOINTNUM];
...
stBoolCrossRecord BoolCrossRecord[HFOREXNUMBER][HCROSSNUMBER+1];
...
void initsymbol()
{
  ...
  else if(AccountServer() == HICMARKETSSERVERDEMO)
  {
    ...
    ForexIndex[29].forexexchangevalue =  1; 
    ForexIndex[30].forexexchangevalue =  MarketInfo(MySymbol[30],MODE_ASK); 


    ForexIndex[31].forexexchangevalue =  MarketInfo(MySymbol[20],MODE_ASK);
    ForexIndex[32].forexexchangevalue =  MarketInfo(MySymbol[20],MODE_ASK);
    ForexIndex[33].forexexchangevalue =  1/MarketInfo(MySymbol[43],MODE_ASK);
    ForexIndex[34].forexexchangevalue =  1/MarketInfo(MySymbol[43],MODE_ASK);
    ForexIndex[35].forexexchangevalue =  1/MarketInfo(MySymbol[46],MODE_ASK);
    ForexIndex[36].forexexchangevalue =  1/MarketInfo(MySymbol[29],MODE_ASK);
    ...
    ForexIndex[44].forexexchangevalue =  1; 
    ForexIndex[45].forexexchangevalue =  1;
    ForexIndex[46].forexexchangevalue =  1; 
    ForexIndex[47].forexexchangevalue =  1; 
    ForexIndex[48].forexexchangevalue =  1;
    ForexIndex[49].forexexchangevalue =  1; 


    ForexIndex[50].forexexchangevalue =  MarketInfo(MySymbol[4],MODE_ASK);
    ForexIndex[51].forexexchangevalue =  1/MarketInfo(MySymbol[26],MODE_ASK);
    ForexIndex[52].forexexchangevalue =  MarketInfo(MySymbol[14],MODE_ASK);
    ForexIndex[53].forexexchangevalue =  MarketInfo(MySymbol[14],MODE_ASK);
    ForexIndex[54].forexexchangevalue =  MarketInfo(MySymbol[14],MODE_ASK);
    ForexIndex[55].forexexchangevalue =  MarketInfo(MySymbol[14],MODE_ASK);
    ForexIndex[56].forexexchangevalue =  MarketInfo(MySymbol[14],MODE_ASK);
    ForexIndex[57].forexexchangevalue =  MarketInfo(MySymbol[14],MODE_ASK);
    ForexIndex[58].forexexchangevalue =  MarketInfo(MySymbol[14],MODE_ASK);
    ForexIndex[59].forexexchangevalue =  MarketInfo(MySymbol[14],MODE_ASK);
    ForexIndex[60].forexexchangevalue =  MarketInfo(MySymbol[20],MODE_ASK);
    ForexIndex[61].forexexchangevalue =  MarketInfo(MySymbol[20],MODE_ASK);
    ForexIndex[62].forexexchangevalue =  MarketInfo(MySymbol[20],MODE_ASK);


    ForexIndex[63].forexexchangevalue =  MarketInfo(MySymbol[63],MODE_ASK); 
    ForexIndex[64].forexexchangevalue =  1;
    ForexIndex[65].forexexchangevalue =  1;
    ForexIndex[66].forexexchangevalue =  1; 
    ForexIndex[67].forexexchangevalue =  1;
    ForexIndex[68].forexexchangevalue =  1; 
...
double autocalculateamount(int SymPos,int buysellpoint,int subbuysellpoint)
{
  ...
  if ((buysellpoint <= 3*HBUYSELLALGNUM)&&(buysellpoint > 0))
  {
     //最小值作为基本单位；
    lastamount = 2*ForexIndex[SymPos].minlot;   
  }
  else
  {
    //最小值作为基本单位；
...
void InitBuySellPos()
{
  ...
  for(i = 0; i < 6*HBUYSELLALGNUM+1; i++)
  {
    ...
    //趋势逆转买卖点
    else if ((i <= (HBUYSELLALGNUM*3+HBUYSELLALGNUM/2))&&(i > HBUYSELLALGNUM*3))
    {

      SubMagicName[i] = "TrendBreakNumber"+IntegerToString(i);        
    }
    else if ((i <= HBUYSELLALGNUM*4)&&(i > (HBUYSELLALGNUM*3+HBUYSELLALGNUM/2)))
    {

      SubMagicName[i] = "TrendBreakNumber"+IntegerToString(i);        
    }
    else if ((i <= (HBUYSELLALGNUM*4+HBUYSELLALGNUM/2))&&(i > HBUYSELLALGNUM*4))
    {

      SubMagicName[i] = "TrendBreakNumber"+IntegerToString(i);        
    }
    else if ((i <= HBUYSELLALGNUM*5)&&(i > (HBUYSELLALGNUM*4+HBUYSELLALGNUM/2)))
    {

      SubMagicName[i] = "TrendBreakNumber"+IntegerToString(i);        
    }   

    else if ((i <= (HBUYSELLALGNUM*5+HBUYSELLALGNUM/2))&&(i > HBUYSELLALGNUM*5))
    {

      SubMagicName[i] = "TrendBreakNumber"+IntegerToString(i);        
    }
    else if ((i <= HBUYSELLALGNUM*6)&&(i > (HBUYSELLALGNUM*5+HBUYSELLALGNUM/2)))
    {
  ...
  for(SymPos = 0; SymPos < symbolNum;SymPos++)
  {
    ...
    for(subbuysellpoint = 0; subbuysellpoint < HSUBBUYSELLPOINTNUM;subbuysellpoint++)
    {
      for(buysellpoint = 1; buysellpoint <= 6*HBUYSELLALGNUM;buysellpoint++)
      {
        ...
        //定义stoptailing为2.1
        BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoptailtimes = 4;

        //定义伪装止损止盈值和实际止损止盈值之间的比值，防止黑平台根据你的头寸恶意止损和止盈
        BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].fakedlevel = 1.01;
        ...
        if ((buysellpoint <= HBUYSELLALGNUM)&&(buysellpoint > 0))
        {
          ...
          BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossleverage = 30;
          //按照1.5倍的止损止盈比计算
...
void ChangeCrossValueL( int mvalue,double  mstrongweak,int SymPos,int timeperiodnum)
{
  ...
  if (mvalue == BoolCrossRecord[SymPos][timeperiodnum].CrossFlagL[0])
  {
...
double  ordersrealprofitalltrend( )
{
  double profit = 0;
  int i,SymPos,NowMagicNumber;
  string my_symbol;
...
//去除近期成交单和设置成无止盈的手工单
double  ordersexpectedmaxprofitalltrend( )
{
  double profit = 0;
  int i,SymPos,NowMagicNumber;
  string my_symbol;
          ...
          if(OrderTakeProfit()>0.01)
          {
            if((TimeCurrent()-OrderOpenTime())>BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].keepperiod)
            {
              ...
              if(OrderType()==OP_BUY)
              {
                ...
                //持有订单时间，单位为天，但是允许小数点，持有订单的期望盈利值随着持有时间的变长，期望值变低；增加了基于时间来衰减的概念，
                timekeep = ((double)((TimeCurrent()-OrderOpenTime())))/(60*60*24);
                //基于时间的衰减指数
                decayvalue = 7/(timekeep*timekeep+7);           
                //加入了时间衰减指数
                profit += decayvalue*(BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].takeprofit - BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].openprice)/
                      BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].openprice;
              }
              ...
              if(OrderType()==OP_SELL)
              {
                ...
                decayvalue = 7/(timekeep*timekeep+7);             
                //加入了时间衰减指数        
                profit += decayvalue*(BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].openprice - BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].takeprofit)/
                      BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].openprice;        
              }
...
bool isvalidmagicnumberbreak(int magicnumber)
{
  ...
  NowMagicNumber = ((int)NowMagicNumber) /100;
  if((NowMagicNumber>HBUYSELLALGNUM*6)||(NowMagicNumber<=HBUYSELLALGNUM*3))
  {
    flag = false;
  } 
...
double  ordersrealprofitallbreak( )
{
  double profit = 0;
  int i,SymPos,NowMagicNumber;
  string my_symbol;
  ...
  for (i = 0; i < OrdersTotal(); i++)
  {
    if (OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
    {
      ...
      {     
        ...
        if((buysellpoint>3*HBUYSELLALGNUM)&&(buysellpoint<=6*HBUYSELLALGNUM))
        {
...
// 期望最大止盈值，是按照盈利百分比来计算的，设想每个单都达到了止盈值的盈利百分比。sum(|ordertakeprofit-orderopenprice|/orderopenprice)
//去除近期成交单和设置成无止盈的手工单
double  ordersexpectedmaxprofitallbreak( )
{
  double profit = 0;
  int i,SymPos,NowMagicNumber;
  string my_symbol;
...
bool isvalidmagicnumberall(int magicnumber)
{
  ...
  NowMagicNumber = ((int)NowMagicNumber) /100;
  if((NowMagicNumber<=0)||(NowMagicNumber>HBUYSELLALGNUM*6))
  {
    flag = false;
  } 
  ...
  for (i = 0; i < OrdersTotal(); i++)
  {
    if (OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
    {
      ...
      {     
        ...
        if((buysellpoint>0)&&(buysellpoint<=3*HBUYSELLALGNUM))
        {
          ...
          if(OrderTakeProfit()>0.01)
          {
            if((TimeCurrent()-OrderOpenTime())>BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].keepperiod)
            {
              if(OrderType()==OP_BUY)
              {
                profit += 2*(vask - BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].openprice)/
                      BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].openprice;
              }
              ...
              if(OrderType()==OP_SELL)
              {
                ...
                profit += 2*(BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].openprice - vask)/
                      BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].openprice;        
              }
        ...
        else if((buysellpoint>3*HBUYSELLALGNUM)&&(buysellpoint<=6*HBUYSELLALGNUM))
        {
...
double  ordersrealprofitrealall( )
{
  double profit = 0;
  int i,SymPos,NowMagicNumber;
  string my_symbol;
...
// 期望最大止盈值，是按照盈利百分比来计算的，设想每个单都达到了止盈值的盈利百分比。sum(|ordertakeprofit-orderopenprice|/orderopenprice)
//去除近期成交单和设置成无止盈的手工单
double  ordersexpectedmaxprofitallall( )
{
  double profit = 0;
  int i,SymPos,NowMagicNumber;
  string my_symbol;
          ...
          if(OrderTakeProfit()>0.01)
          {
            if((TimeCurrent()-OrderOpenTime())>BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].keepperiod)
            {
              ...
              if(OrderType()==OP_BUY)
              {
                ...
                decayvalue = 7/(timekeep*timekeep+7);           
                //加入了时间衰减指数
                profit += 2*decayvalue*(BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].takeprofit - BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].openprice)/
                      BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].openprice;
              }
              ...
              if(OrderType()==OP_SELL)
              {
                ...
                decayvalue = 7/(timekeep*timekeep+7);             
                //加入了时间衰减指数        
                profit += 2*decayvalue*(BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].openprice - BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].takeprofit)/
                      BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].openprice;        
              }
  ...
  for (i = 0; i < OrdersTotal(); i++)
  {
    if (OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
    {
      ...
      {     
        ...
        if((buysellpoint>0)&&(buysellpoint<=6*HBUYSELLALGNUM))
        {
          //当去掉止盈的时候，程序对该单放弃监控，转为手动监控，通常是指那些基本面同步发生了重大同方向的变化，且适合长期持有的单子；改为手工持单
          //一般情况下不触发
          if(OrderTakeProfit()>0.01)
...
void monitoraccountprofittrend()
{
  ...
  allordernumbers = ordercountallall( );
  /*当天订单已经平掉的情况下就不走这个分支了*/
  if(allordernumbers<=4)
  {
    return;
  }


  //巡视趋势单的盈利状况，趋势单平仓时将所有的反趋势单子一并平仓，降低回调过大导致的问题;最恶劣的情况是趋势一直反转，那就只能是触发不断止损。
  allordernumbers = ordercountalltrend( );
  if(allordernumbers>2)
  {
    if(ordersrealprofitalltrend( )>(ordersexpectedmaxprofitalltrend( )/(2*((allordernumbers/symbolNum)*(allordernumbers/symbolNum)+(allordernumbers/symbolNum)+1))))
    {
      /*turnoffflag = true;*/           
      Print("Trend successfully Close All ordersexpectedmaxprofitall = " + ordersexpectedmaxprofitalltrend( )
          +"ordersrealprofitall = "+ordersrealprofitalltrend( ));
      ...
      {     
        ...
        if(k>=(j-1))
        {   
          Print("!!monitoraccountprofittrend close all Something Serious Error by colse all order,pls close handly");     
  ...
  //巡视反趋势单的盈利状况,反趋势的单子更容易平仓
  allordernumbers = ordercountallbreak( );
  if(allordernumbers>2)
  {
    if(ordersrealprofitallbreak( )>(ordersexpectedmaxprofitallbreak( )/(4*((allordernumbers/symbolNum)*(allordernumbers/symbolNum)+(allordernumbers/symbolNum)+1))))
    {
      /*turnoffflag = true;*/           
      Print("Break successfully Close All ordersexpectedmaxprofitall = " + ordersexpectedmaxprofitallbreak( )
          +"ordersrealprofitall = "+ordersrealprofitallbreak( ));
      /*关闭所有在监控的货币，去掉止盈的货币和近期刚进入的货币不在监控范围内*/
      //if(turnoffflag == true)
      {     
        j=0;
        ...
        if(k>=(j-1))
        {   
          Print("!!monitoraccountprofitbreak close all Something Serious Error by colse all order,pls close handly");     
          //SendMail( "!!monitoraccountprofittrend Something Serious Error by colse all order,pls close handly","");    
        }
  ...
  if(allordernumbers>2)
  {
    ...
    /*确保交易时间段，来临前开启全局交易交易标记，主要是规避库存的问题，在不亏损的情况下清仓，规避库存收费的问题*/
    if ((TimeHour(timelocal) >= 18 )&& (TimeHour(timelocal) <=22 )) 
    { 
      ...
      {   
        ...
        if((ordersrealprofitallall( )>(ordersexpectedmaxprofitallall( )/(10*((allordernumbers/symbolNum)*(allordernumbers/symbolNum)+(allordernumbers/symbolNum)+1))))
          &&(ordersrealprofitrealall()>0))
        {
          /*turnoffflag = true;*/           
          Print("All successfully Close All ordersexpectedmaxprofitall = " + ordersexpectedmaxprofitallall( )
              +"ordersrealprofitall = "+ordersrealprofitallall( ));
          /*关闭所有在监控的货币，去掉止盈的货币和近期刚进入的货币不在监控范围内*/
          //if(turnoffflag == true)
          {     
            j=0;
            ...
            if(k>=(j-1))
            {   
              Print("!!monitoraccountprofitall close all Something Serious Error by colse all order,pls close handly");     
              //SendMail( "!!monitoraccountprofittrend Something Serious Error by colse all order,pls close handly","");    
            }
    ...
    if((ordersrealprofitallall( )>(ordersexpectedmaxprofitallall( )/(6*((allordernumbers/symbolNum)*(allordernumbers/symbolNum)+(allordernumbers/symbolNum)+1))))
      &&(ordersrealprofitrealall( )>0))
    {
      /*turnoffflag = true;*/           
      Print("All successfully Close All ordersexpectedmaxprofitall = " + ordersexpectedmaxprofitallall( )
...
//开反向对冲单
void sendopsitebuysellorder(int mySymPos,int mybuysellpoint,int mysubbuysellpoint,int timeperiodnum)
{
  ...
  if((mybuysellpoint<=0)||(mybuysellpoint>HBUYSELLALGNUM*3))
  {
    Print("sendopsitebuysellorder input wrong mybuysellpoint ="+mybuysellpoint);
    return ;
  }   

  if(BuySellPosRecord[mySymPos][mybuysellpoint][mysubbuysellpoint].buysellflag ==1)
  {
    buysellpoint = mybuysellpoint + HBUYSELLALGNUM*3 + 1;
  }
  else
  {
    buysellpoint = mybuysellpoint + HBUYSELLALGNUM*3 - 1;
  ...
  {
    Print("sendopsitebuysellorder error wrong subbuysellpoint ="+subbuysellpoint);    
    return;
  }
  ...
  boll_up_B = iBands(my_symbol,timeperiod[0],iBoll_B,2,0,PRICE_CLOSE,MODE_UPPER,1);   
  boll_low_B = iBands(my_symbol,timeperiod[0],iBoll_B,2,0,PRICE_CLOSE,MODE_LOWER,1);
  boll_mid_B = (boll_up_B + boll_low_B )/2;
  bool_length =(boll_up_B - boll_low_B )/2;
  ...
  if(true == accountcheck())
  {
    ...
    while((ticket<0)&&(ttick<20))
    {
      ...
      else
      {
        ...
        orderTakeProfit = orderPrice - bool_length*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossleverage*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossprofitleverage;
        fakedTakeprofit = orderPrice - bool_length*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossleverage*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossprofitleverage*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].fakedlevel;
      

        /*针对止盈太大导致出现负数的情况进行修正*/
        if(fakedTakeprofit <= 0)
        {
          orderTakeProfit =orderPrice -   bool_length*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossleverage*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossprofitleverage*0.8;    
          fakedTakeprofit = orderPrice - bool_length*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossleverage*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossprofitleverage*0.8*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].fakedlevel;

        }
        if(fakedTakeprofit <= 0)
        {
          orderTakeProfit =orderPrice -   bool_length*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossleverage*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossprofitleverage*0.8*0.8;    
          fakedTakeprofit = orderPrice - bool_length*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossleverage*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossprofitleverage*0.8*0.8*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].fakedlevel;
        
        }
        if(fakedTakeprofit <= 0)
        {
          orderTakeProfit =orderPrice -   bool_length*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossleverage*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossprofitleverage*0.8*0.8*0.8;    
          fakedTakeprofit = orderPrice - bool_length*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossleverage*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossprofitleverage*0.8*0.8*0.8*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].fakedlevel;

        }
        if(fakedTakeprofit <= 0)
        {
          orderTakeProfit =orderPrice -   bool_length*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossleverage*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossprofitleverage*0.8*0.8*0.8*0.8;    
          fakedTakeprofit = orderPrice - bool_length*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossleverage*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossprofitleverage*0.8*0.8*0.8*0.8*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].fakedlevel;

        }
        if(fakedTakeprofit <= 0)
        {
          orderTakeProfit =orderPrice -   bool_length*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossleverage*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossprofitleverage*0.8*0.8*0.8*0.8*0.8;    
          fakedTakeprofit = orderPrice - bool_length*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossleverage*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossprofitleverage*0.8*0.8*0.8*0.8*0.8*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].fakedlevel;

        }
        if(fakedTakeprofit <= 0)
        {
          orderTakeProfit =orderPrice -   bool_length*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossleverage*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossprofitleverage*0.8*0.8*0.8*0.8*0.8*0.8;    
          fakedTakeprofit = orderPrice - bool_length*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossleverage*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].stoplossprofitleverage*0.8*0.8*0.8*0.8*0.8*0.8*BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].fakedlevel;
      ...
      else
      {
       ...
       {       
        ...
        Print("OrderSend Break"+BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].MagicName+"  successfully");
       }                          
      Sleep(1000);  
    }
...
void sendbuysellorder(int SymPos,int timeperiodnum,int buysellpoint,int subbuysellpoint,datetime timeexp)
{
  ...
  //同一个买卖点要间隔15分钟以上，防止由于抖动或者平台不确定性出现相近时刻多个雷同交易单的出现
  if((TimeCurrent() - getlatestsubbuysellopentime(SymPos,buysellpoint)) < 60*15)
  {
    return;
  }
  ...
  if(BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].buysellflag ==1)
  {
    //防止数据市，大起大落引起的买单，考虑到有些货币对点差比较大，因此不用中轴线
    if(vbid > boll_up_B)  
    {
  ...
  if(BuySellPosRecord[SymPos][buysellpoint][subbuysellpoint].buysellflag ==1)
  {
    //逆海龟算法，趋势下盈利持续加仓法，规避黑天鹅风险，且趋势下来狂赚
    //海龟算法，逆趋势加仓，但是间距更大，摊薄买入点不佳导致的亏损      
    if((vask < (orderbuymaxopenpricetrend(SymPos)+bool_length*0.5))&&(vbid >(orderbuyminopenpricetrend(SymPos)-bool_length*1.5)))
    {
      //return;
    }
  }
  else
  {
    //逆海龟算法，趋势下盈利持续加仓法，规避黑天鹅风险，且趋势下来狂赚  
    //海龟算法，逆趋势加仓，但是间距更大 ，摊薄买入点不佳导致的亏损       
    if((vbid > (ordersellminopenpricetrend(SymPos)-bool_length*0.5))&&(vask < (ordersellmaxopenpricetrend(SymPos)+bool_length*1.5)))
    {
     // return;
    } 
  ...
  //下对冲单，反趋势，对冲后续持续反趋势的情况
  sendopsitebuysellorder(SymPos,buysellpoint,subbuysellpoint,timeperiodnum);
...
void orderbuyselltypetrend(int SymPos,int timeperiodnum)
{
  ...
  //多头大周期维持趋势不变处于双周期上涨加速阶段，在奔赴一个新的平衡过程中，小周期择低买入
  ...
  //大周期加速上涨，更大周期由下跌周期转型为上涨周期，小周期择低买入
  if((BoolCrossRecord[SymPos][timeperiodnum+2].StrongWeak>0.8)  
    &&(BoolCrossRecord[SymPos][timeperiodnum+3].StrongWeak<0.8) 

    && (3< BoolCrossRecord[SymPos][timeperiodnum+2].CrossFlag[0]) 
    && (0< BoolCrossRecord[SymPos][timeperiodnum+3].CrossFlagL[0])  
  ...
  if((BoolCrossRecord[SymPos][timeperiodnum+2].StrongWeak>0.8)  
    &&(BoolCrossRecord[SymPos][timeperiodnum+3].StrongWeak<0.8)     
    && (3< BoolCrossRecord[SymPos][timeperiodnum+2].CrossFlag[0]) 
    && (3< BoolCrossRecord[SymPos][timeperiodnum+3].CrossFlagL[0])  
