...
stBuySellPosRecord BuySellPosRecord[50][3*HBUYSELLALGNUM+1][8];

string SubMagicName[3*HBUYSELLALGNUM+1];
...
void InitBuySellPos()
{
	...
	/*
	SubMagicName[1] = "TrendBreakNumber01";
	SubMagicName[2] = "TrendBreakNumber02";
	SubMagicName[3] = "TrendBreakNumber03";
	SubMagicName[4] = "TrendBreakNumber04";
	SubMagicName[5] = "TrendBreakNumber05";
	SubMagicName[6] = "TrendBreakNumber06";
	SubMagicName[7] = "TrendBreakNumber07";
	SubMagicName[8] = "TrendBreakNumber08";
	SubMagicName[9] = "TrendBreakNumber09";
	SubMagicName[10] = "TrendBreakNumber10";
	SubMagicName[11] = "TrendBreakNumber11";
	SubMagicName[12] = "TrendBreakNumber12";
	SubMagicName[13] = "TrendBreakNumber13";
	SubMagicName[14] = "TrendBreakNumber14";
	SubMagicName[15] = "TrendBreakNumber15";
	SubMagicName[16] = "TrendBreakNumber16";
	SubMagicName[17] = "TrendBreakNumber17";
	SubMagicName[18] = "TrendBreakNumber18";
	SubMagicName[19] = "TrendBreakNumber19";
	SubMagicName[20] = "TrendBreakNumber20";
	SubMagicName[21] = "TrendBreakNumber21";
	SubMagicName[22] = "TrendBreakNumber22";
	SubMagicName[23] = "TrendBreakNumber23";
	SubMagicName[24] = "TrendBreakNumber24";
	...
	*/
	for(SymPos = 0; SymPos < symbolNum;SymPos++)
	{
		...
		for(subbuysellpoint = 0; subbuysellpoint <= 7;subbuysellpoint++)
		{
			for(buysellpoint = 1; buysellpoint <= 3*HBUYSELLALGNUM;buysellpoint++)
			{
   		...
   		if (OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
   		{
			...
			if((SymPos>=0)&&(SymPos<symbolNum))
			{
				...
				if((subbuysellpoint>= 0)&&(subbuysellpoint<= 7))
				{
					...
					if((buysellpoint>=1)&&(buysellpoint<=HBUYSELLALGNUM*3))
					{
...
//去除近期成交单和设置成无止盈的手工单
double  ordersexpectedmaxprofitall(int timeperiodnum )
{
	double profit = 0;
	int i,SymPos,NowMagicNumber;
	string my_symbol;
...
int ordercountall( int timeperiodnum)
{
	int count = 0;
	int i,SymPos,NowMagicNumber;
	string my_symbol;
...
void ordercloseall(int timeperiodnum)
{
...
// 正常交易单全部关闭掉；去除近期成交单和设置成无止盈的手工单
void ordercloseall2(int timeperiodnum)
{
	int i,SymPos,NowMagicNumber,ticket;
	string my_symbol;
	double vbid,vask;
...
double  ordersrealprofitallbyforex( int MySymPos)
{
	double profit = 0;
	int i,SymPos,NowMagicNumber;
	string my_symbol;
...
// 期望最大止盈值，是按照盈利百分比来计算的，设想每个单都达到了止盈值的盈利百分比。sum(|ordertakeprofit-orderopenprice|/orderopenprice)
//去除近期成交单和设置成无止盈的手工单
double  ordersexpectedmaxprofitallbyforex(int MySymPos )
{
	double profit = 0;
	int i,SymPos,NowMagicNumber;
	string my_symbol;
...
// 交易单的总数量，去除近期成交单和设置成无止盈的手工单
int ordercountallbyforex( int MySymPos)
{
	int count = 0;
	int i,SymPos,NowMagicNumber;
	string my_symbol;
...
// 正常交易单全部关闭掉；去除近期成交单和设置成无止盈的手工单
void ordercloseallbyforex(int MySymPos)
{
	int i,SymPos,NowMagicNumber,ticket;
	string my_symbol;
	double vbid,vask;
...
// 正常交易单全部关闭掉；去除近期成交单和设置成无止盈的手工单
void ordercloseall2byforex(int MySymPos)
{
	int i,SymPos,NowMagicNumber,ticket;
	string my_symbol;
	double vbid,vask;
...
void monitoraccountprofit()
{
   ...
   int SymPos;
	...
	allordernumbers = ordercountall(-1);
	...
	for(timeperiodnum = -1; timeperiodnum < 3;timeperiodnum++)
	{
		...
		if(allordernumbers>2)
		{
			if(ordersrealprofitall(timeperiodnum)>(ordersexpectedmaxprofitall(timeperiodnum)*50/(allordernumbers*allordernumbers+10*allordernumbers+50)))
			{
				...
				//turnoffflag = true;						
				Print("2 successfully Close All"+timeperiodnum+": allordernumbers = " + allordernumbers + "ordersexpectedmaxprofitall = " + ordersexpectedmaxprofitall(timeperiodnum)
				...
				{			
					...
					if(k>=(j-1))
					{		
						Print("!!monitoraccountprofit "+timeperiodnum+"Something Serious Error by colse all order,pls close handly");			
	...
	for(SymPos = 0; SymPos < symbolNum;SymPos++)
	{
		...
		allordernumbers = ordercountallbyforex(SymPos);
		if(allordernumbers>2)
		{
			if(ordersrealprofitallbyforex(SymPos)>
				(ordersexpectedmaxprofitallbyforex(SymPos)*100/(2*allordernumbers*allordernumbers*allordernumbers+allordernumbers*allordernumbers+10*allordernumbers+100)))
			{
...
void orderbuyselltype(int SymPos,int timeperiodnum)
{
...
void OnTick(void)
{
	...
	{	
		/*特定货币一分钟寻找买卖点*/
		orderbuyselltype(SymPos,0);		
				
		/*特定货币五分钟寻找买卖点*/		
		orderbuyselltype(SymPos,1);		
		
		/*特定货币三十分钟寻找买卖点*/
		orderbuyselltype(SymPos,2);					
